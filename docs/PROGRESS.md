...
源对比表」两个含义。已入库用户点「重新获取清单」时，`reacquire()` 把 pkg
置空而 installed 仍为 true，表格无处可去，页面落回状态 C——用户看到的是
「本地没有清单内容」加一个「重新获取以编辑」按钮，而后台其实正在 lookup。
已加 `reacquiring` 意图标记拆开两义，并补「取消，保持现状」退路。
该缺陷先于 08-02 那四个问题存在，从未被记录过。

**问题 1 的根因（已修，且原判断低估了严重性）**：`watch(appID)` 中
`pkg.value = null` 同步执行，而 `installed` 来自 `library.find(appID)` 也
同步变 true，命中状态 C 的模板条件 `installed && !pkg`。

⚠️ 但它**不是「闪一帧」**：`loadStored()` 是异步的，且 `load()` 会先
`await getGameDetail()` 才轮到它，所以状态 C 在整个等待期间**真实渲染**，
按网络状况可达数秒。而那段文案写着「本地没有这个游戏的清单内容」并建议
重新获取——在尚未查明时对用户断言了结论。

修法：新增 `storedLoading`，插入明确的「正在读取本地清单」状态排在状态 C
之前，状态 C 判据收紧为 `installed && !storedLoading`。标记在四处同步置位
（setup 顶层取 `installed` 初值、`watch(appID)` 回调内、`load()` 首个 await
之前、`loadStored()` 入口），漏掉任一处都会重现。

另修一个同源问题：以 URL 直入库内游戏时 `library.refresh()` 尚未返回，
`installed` 初值为 false，`load()` 走了未入库分支；等 store 回填后
`installed` 迟到翻 true，而那次 `load` 早已结束——留存读取根本不会发生，
页面停在状态 C。已加 `watch(installed)` 补读一次。
...
**问题 3 的实情与描述不同（已修）**：入口原本已存在于 `LibraryShell` 侧栏
footer。但事实上没被找到 = 等于没做。三个可能原因：在侧栏最底部需滚到底；
紧贴「折叠」开关而两者都是低视觉重量的灰色条目；「库概览」是页面名而非
回退动作。且与 `HomeShell` 不一致——后者把同语义的「在线搜索」放在**顶部**
成组。同一语义角色在两壳中位置相反，违反宪法 3.1。

修法是**移位而非新增**：移到侧栏顶部新建的「总览」分组，并补 `meta`
（「总计与异常检查」，说清它是什么而非只有页面名）与 `hasAnomaly` 警示。

### 🔜 后续路线（按此顺序）

1. ✅ 验证搜索超时根因（08-02，见 DECISIONS-2）
2. ✅ 一夜采样 341 次，方案据此修正（08-03）
3. ✅ 修 `GamePane` 状态 C 误报（08-03）
4. ✅ 库概览入口移到侧栏顶部（08-03）
5. ✅ **多样本测试定性问题 4**（08-03，全国二十余人）——
   降级为文案议题，并入第 9 项，见下方「问题 4 定性」
6. 搜索与库页面状态缓存
7. 切页后工作继续（请求生命周期移出组件）
8. UI 第 5 步剩余：`SearchPane` 原生控件、`DlcList` checkbox 性能实测
9. UI 第 5 步剩余：`TopBar` 下划线改常驻指示器、花纹投放、LOGO
10. UI 第 6 步：术语帮助系统、键盘可达性、
    **搜索空态与错误文案（含超时数值与错误分类，问题 4 归此）**
11. 回补文档

排序依据：问题 4 已定性为「本机 / 线路的偶发现象，多数用户不受影响」，
不再是阻塞项，故从第 2 位挪到 UI 收尾阶段——它现在要解决的是「用户遇到时
知道发生了什么」，而非「让它不发生」。
问题 2（第 6、7 项）成为当前最大的架构改动，牵动状态归属，须单独做。

### 🔬 问题 4 定性（08-03 结论，已结案）

**结论：这是本机 / 线路的偶发现象，不是盒子的缺陷，也不是普遍现象。**
测试群约二十余人、来自全国各地的多样本测试显示：偶发失败确实存在，但
**主人这种重度失败（成功率 54%）反而罕见**，且成因不在本工具这一侧。

**故问题 4 的处置方式整体改变**：不再作为待修 bug 排在最前，而是降级为
**文案议题**，并入 UI 第 6 步收尾一并做。要解决的问题从「让它不发生」
变成「用户遇到时知道发生了什么、下一步该做什么」。

搜索首页本来就有大片空白，正是安置这类说明的地方——**空白不是留白位，
是引导位**（宪法 3.5 第 3 条对内容区的要求，同样适用于搜索首页）。

具体要做的三件事（归入路线第 10 项）：

1. 搜索首页空态写明可能的情况：网络波动导致的失败是偶发的、与本工具无关、
   稍等一两分钟再试通常就好，以及可以直接粘 AppID 走更稳的路径。
2. 错误按类型给不同提示，而非统一一句「搜索失败」。分类逻辑可直接复用
   `tools/netprobe/main.go` 的 `classify`（七个分支）。
3. 超时数值一并调整（见下方采样数据），但它现在是文案工作的附带项，
   不再是独立的修复任务。

⚠️ **`repo_client.go` 的两个 client 依旧不动**，理由见下方「结论三」。

---

以下是 08-03 的采样数据与推论，**结论已被上述多样本测试覆盖**，但数据本身
长期有效：定超时数值、写错误文案都要用到它，且它记录了几个可复用的一般判断。

`go run .\tools\netprobe -watch 1m` 挂了约 6 小时，成功 185 / 失败 156，
成功率 54.3%。失败分类：155 次「等响应头超时」，1 次 DNS 解析失败。
⚠️ 这个成功率现已确认为**主人本机的极端值**，不可当作用户普遍水平。

**四条改变方案的事实**：

1. **故障是成块的，块长以分钟计。** 按成功时间戳的间隔反推（采样间隔恒为
   1 分钟且 `time.NewTicker` 无漂移），29 个故障块的长度为
   `2,4,4,4,2,2,2,2,31,12,5,5,12,12,9,8,1,1,8,5,5,1,2,12,2,11,11,5,2` 分钟——
   只有 3 个是单采样点，最长 31 分钟。

2. **故此「重试 2 次、间隔 500ms/1.5s」不成立。** 26/29 的故障块 ≥2 分钟，
   2 秒内的重试百分之百落在同一块里，代价是把 6s 等待变成
   `6+0.5+6+1.5+6 ≈ 20s`，比现状 15s 更糟。
   ⚠️ 但采样间隔是 1 分钟，**分辨不出分钟以下的结构**——所谓「1 分钟的块」
   也可能只是 5 秒的抖动。准确表述是「没有证据支持 2 秒级重试有效，
   且有强证据表明分钟级故障占 90%」。故改为 **重试 1 次、间隔 800ms**：
   最坏 `6+0.8+6 ≈ 12.8s` 仍短于现状，能兜住真正瞬时的抖动。

3. **6s 超时的判据比原来更硬。** p50=312ms，p95=642ms，超 2s 的仅 3 个样本。
   两个超过 6s 的成功（7.533s、7.576s）经查上下文，**都是故障块中间的孤立
   成功**，前后全是失败——它们不是慢样本，是故障边缘的挣扎，本就该判失败。
   原备忘「正常上限 2.5s」可升级为「p95=642ms」。
   同时 `searchTypeProbeTimeout = 12s` 也须砍（原计划漏了它）：故障块边缘
   若 storesearch 侥幸成功，接着这批探查全灭，用户额外白等 12 秒——
   **当前最坏路径是 15+12=27 秒**。

4. **`isNumeric(term)` 那条既有分支是这种网络下最稳的路。** 纯数字输入直接
   当 AppID 查详情，跳过 storesearch，请求数从 `1+N` 降到 1。在 54% 成功率
   下这是数量级差别，而用户不知道它存在——又是「功能在那儿而用户找不到」。
   修法是搜索框占位文字与失败提示都标明可直接粘 AppID，零后端改动。

**`repo_client.go` 的两个 client 本轮不动**，与原备忘的计划相反。
`fetchTimeout=120s` 是下载，长是对的；`lookupTimeout=15s` 的注释里写了明确
依据（codeload 直连实测 21 秒才报失败，砍短会让「网络慢」变成「探测不明」，
进而在下载阶段白走四级镜像链）。而这一夜的 341 次采样**全打在
`store.steampowered.com`，一次都没打 codeload**。改它需要先对 codeload 做
一轮同样的采样，否则就是拿 A 的证据改 B。

**一个仍未定的假设**：09:34 主人实测「浏览器秒开而盒子必失」，但十分钟后
盒子自己好了——所以那次对照测到的是「当时在故障块内」，不能证明 Go 栈与
浏览器有差异。真实存在且能解释该现象的假设是：浏览器有到 store 域名的
**热连接**（keep-alive + HTTP/2 多路复用），而盒子每次搜索都新建连接；
若丢包发生在新连接的首个请求上，浏览器天然免疫。
判据：给 client 做连接预热，若失败率明显下降则假设成立，无变化则是纯 IP
层丢包、与栈无关。

⚠️ **该假设现已不值得验证**：多样本测试显示多数用户不受影响，说明问题不在
「盒子每次新建连接」这个所有用户共有的行为上——若真是热连接的差异，
所有人都该复现。热连接假设**因此被间接否证**，不必再做预热实验。
保留这段记录是因为那条一般判断仍然有效（见下）。

**用户环境的判断也被修正了。** 08-03 上午咱写下「主人处为四川 + 移动，
这不是本机问题而是目标用户的真实环境」——**这条被同日的多样本测试推翻**：
二十余人分布全国各地，重度失败反而罕见。

那个错误判断的形成过程值得记下：从「四川 + 移动」这个真实事实出发，
推出「国内玩家网络条件普遍如此」，听起来合理，但**样本量始终是 1**。
把 n=1 的观测冠以「目标用户的真实环境」，只是给单点数据换了个说法，
并不会让它变成分布。教训：**「这是普遍现象」这个判断本身需要多样本证据，
不能由「这个用户是典型用户」推出。**
