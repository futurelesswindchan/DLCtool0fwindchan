...
## ⚠️ 当前阶段：UI 第 5 步部分完工，四个实机问题待修（分支 `feat/ui-v2`）
...
| # | 问题 | 性质 | 根因是否已定位 |
| :-- | :--- | :--- | :--- |
| 1 | 切换游戏详情闪一下状态 C | 前端时序 | ✅ 已定位 |
| 2 | 切页丢失页面状态与进行中的工作 | 前端架构 | ✅ 已定位 |
| 3 | 库界面回不到库概览 | 前端可发现性 | ✅ 已定位，且**入口其实已存在** |
| 4 | 搜索频繁超时 | 间歇性网络故障 | ⚠️ 已排除代理因素，真因待长时间采样 |

**问题 1 的根因**：`GamePane` 的 `watch(appID)` 中 `pkg.value = null` 是同步
执行，而 `installed` 来自 `library.find(appID)` 也同步变 true。于是切到
已入库游戏 B 时存在一帧 `pkg=null && installed=true`，精确命中状态 C 的
模板条件 `installed && !pkg`。随后 `loadStored()` 异步读回才跳状态 B。

⚠️ 注意「**无过渡动画**」不是缺陷：`PaneTransition` 注释明确写着同组件换
appID 时实例复用、无过渡，这是宪法 5.2 刻意要的效果（侧栏指示器滑移、
右侧就地换内容，而非整块闪一下）。只需修「闪状态 C」。

**问题 2 的根因**：`SearchPane` 把 `term` / `results` / `searching` /
`searched` 全放组件内 `ref`，切页即随组件销毁。该文件顶部注释
「搜索结果不进 store：属会话级临时数据」是当时有意的决定，现被实机推翻。
须分两层：状态缓存（纯前端）与切页后工作继续（请求生命周期须移出组件）。

**问题 3 的实情与描述不同**：入口已存在于 `LibraryShell` 侧栏 footer
（label「库概览」，`icon="📊"`，`exact`）。但事实上没被找到 = 等于没做。
三个可能原因：在侧栏最底部需滚到底；紧贴「折叠」开关而两者都是低视觉
重量的灰色条目；「库概览」是页面名而非回退动作。
且与 `HomeShell` 不一致——后者把同语义的「在线搜索」放在**顶部**成组。
同一语义角色在两壳中位置相反，违反宪法 3.1。
**修法是把它提到侧栏顶部成组，而非再加一个入口。**

### 🔜 后续路线（10 项，按此顺序）

1. ✅ 验证搜索超时根因（已完成，见 DECISIONS-2）
2. 修搜索：超时 15s→6s、失败重试 2 次、错误分类后给可行动提示
3. 修 `GamePane` 闪状态 C
4. 库概览入口提到侧栏顶部，与 `HomeShell` 语义对齐
5. 搜索与库页面状态缓存
6. 切页后工作继续（请求生命周期移出组件）
7. UI 第 5 步剩余：`SearchPane` 原生控件、`DlcList` checkbox 性能实测
8. UI 第 5 步剩余：`TopBar` 下划线改常驻指示器、花纹投放、LOGO
9. UI 第 6 步：术语帮助系统、键盘可达性
10. 回补文档

排序依据：问题 4 优先级最高，搜索是入口功能。1、3 为小改动可快速清掉。
问题 2 是最大的架构改动，牵动状态归属，须单独做。
UI 剩余欠账是「看起来更好」，让位于「用起来不对」。

---

## UI 施工第 5 步：控件自绘归零（2026-08-02）
...
已验产物：`dist` 内无 `@layer ui` 块，`domain` 与 `utilities` 仍正常注册。

顺手修的两处：`SetupView` 底部两个跳同一路由的按钮合并为一个
（原 `v-if/v-else` 行为完全相同只有文案不同）；补
`.step__desc + .step__desc` 段间距（步骤三两段说明原本贴在一起）。

### 尚未完成的第 5 步欠账

`SearchPane` 原生 input/button、`TopBar` 页签下划线（现为 `animation`）、
`TopBar` `.env` 全圆胶囊（**此处是状态指示灯，胶囊形恰好区分于方角控件，
若统一应去边框只留色点，不是改方角**）、`GameCard` `layout="grid"` 零调用点
待删、`DlcList` 原生 checkbox（条目可上百，性能需实测）、花纹投放、LOGO。

---

## 历史阶段：功能全部完工，待 UI 翻新（分支 `feat/ui-v2`）
...
缺陷③ 与议题⑤ 一并解决。测试由 93 增至 **113 条 PASS**。

原先 `lookupRepos` 只返回源名，用户看到「7 个源收录了该游戏」却可能拿到
0 个 DLC。改为 `trialSources`：真下载、真解析、把各源实得数摆出来让用户自选。

**实机验证**（MHW 582010）：MAU 64 DLC、tymolu233 63、hansaes 与快照源
「仅本体」、MAU 镜像与 bingyu50 报「打包结构不适用」、手动触发 Hubcap 得
200 DLC。选定后日志确认 `复用试下载产物入库`，零二次下载。全源试取约 41 秒。

三个新方法：`TrialSources`（全源，自动跳过认证源）、`TrialOneSource`
（单源，供用户主动花额度）、`InstallFromTrial`（用缓存产物入库，缺失则回退）。

关键取舍见 DECISIONS-2「试下载对比表落地」一条。其中最需防回归的是
**`trialOne` 必须复用 `DownloadFromRepo`**——自行实现会造出第三条解析路径，
其行为差异迟早表现为「对比表说 64 个，装上只有 60 个」。

**同时明确了一条职责认识**：OST 可独立运行且与本工具完全解耦，故本工具的
价值不在部署那一步，而在把「获取一份内容尽可能完整的 lua」的成本降到足够低。
由此，**持续寻找、实测、收录清单源是本项目的核心长期任务**，优先级不低于
功能开发。详见 DECISIONS-2 同名决策，其中记录了由此浮现的架构议题
（源列表当前硬编码，新增源需改代码发版）。
