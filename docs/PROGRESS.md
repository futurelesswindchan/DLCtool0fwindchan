...
## 当前阶段：⑤ 完工并通过实机验证，⑧ 待施工（分支 `refactor/v2`）
...
- [ ] **`readDeployedLua` 按文件名后缀定位，须改为按内容匹配**（必修缺陷）。
      现以 `_<AppID>.lua` 后缀扫描，只能识别盒子自己生成的命名。用户手动放置的
      `2399830.lua` 匹配失败后 `detectInstalledDLCs` 静默走入「未部署过」分支，
      界面显示全部 DLC 未安装而 Steam 中实际已装。改为扫全目录逐个正则匹配
      `addappid(<mainAppID>` 即可，且**顺带获得卸载冲突检测与外部清单对账
      所需的能力——三件事实为同一个函数**
- [ ] 数据目录迁至 exe 同级，`wails.json` dev 配置补 `KAZEUSA_DEV=1`。
      **不实现旧数据迁移**：v1.4 时代（时称 dlctool）不产生任何本地数据文件，
      其操作直接作用于 Steam，故「迁移旧数据」一事并无对象。新位置为空即视为新装
- [ ] `constants.go` 清理 8 个 v1.4 遗留常量（**疑似已完成，待确认后勾除**）
- [ ] 日志输出中的 depot 密钥截断为前 8 位
- [ ] `GameRecord` 增补 `Source` 字段，记录清单来源（M 站 / MAU / 本地导入）
- [ ] `types.go` 中 `DepotInfo` / `DLCInfo` 的注释仍称「需写入 config.vdf」，
      与 v2.0「不碰 config.vdf」的铁律冲突（⑤ 阶段未及处理，顺延至 ⑧ 前）

### 📌 尚未讨论的议题

- [ ] **新压缩包如何覆盖旧版本**。已提出但未展开。与 `GameRecord` 的字段增补
      相关：新增字段在旧 `history.json` 中缺失时应给默认值而非报错
- [ ] 教程草稿待补 OST 蓝奏云链接、界面截图、在线仓库说明、M 站 API key
      申请指引，以及 **ST 残留清理清单与卸载安装顺序**（2026-07-28 新增）
