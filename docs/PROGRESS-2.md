...
>
> 最后更新：2026-08-10（08-09 修复小结 + 1.2/2.1/4.1 进展登记）
...
以下于 08-09/10 修复批次后更新。

### 零、全局样式

| #   | 项目                   | 说明                                                                                                                                                                                                                                                                        | 状态 |
|:---|:---|:---|:---|
| 0.1 | 书脊圆角色条统一 | Toast / EnvBanner / UiHelpBadge 的 `border-left-width` 加宽方案改为 `::before` 伪元素书脊（`border-radius: 0 2px 2px 0`，上下缩进离开圆角弧线），颜色经 `--spine-color` CSS 变量下发。实机验收通过（08-09/10）。 | ✅ |
| 0.2 | 侧栏 emoji icon        | ~~全部换 Font Awesome 或全去掉。~~ 已由 `UiIcon` 替代（第 7 步）。 | ✅ |
| 0.3 | 侧栏展开时内容跳动     | ~~收起→展开时 emoji 与文字多行→单行跳动。~~ 已修（第 7 步）。 | ✅ |
| 0.4 | 侧栏 Y 轴高度固定      | ~~折叠/展开高度不一致。~~ 已修（第 7 步）。 | ✅ |
| 0.5 | 侧栏 brand 图标折叠对齐 | 已修：`left: 20px`（08-09）。根本原因是 `.sidebar__body` 横向内边距，见 0.6。 | ✅ |
| 0.6 | 侧栏 body 横向内边距致图标偏右 | 已修：body 横向改回 `var(--space-2)=8px`，条目 `padding-inline: 20px→12px`，图标中心统一 x=28（08-09）。 | ✅ |
| 0.7 | SidebarItem 选中高亮变大 + 竖条贴边 | 0.6 改动的副作用：body 曾改为 `padding:0` 导致条目撑满 240px，高亮区变大且选中竖条贴侧栏左边缘。随 0.6 一并修复（08-09）。 | ✅ |

### 一、Home 页面

| #   | 项目                   | 说明                                                                                                                                                                                                             | 状态 |
|:---|:---|:---|:---|
| 1.1 | 侧栏顶部与 TopBar 冲突 | ~~LOGO + 昵称重复。~~ 已修（第 7 步）。 | ✅ |
| 1.2 | 搜索栏下方公告区 | idle 态（未搜索）下显示三张书脊卡：公告卡（v2.0.0 寄语，accent 书脊）+ 链接卡（GitHub/博客/B站/爱发电，neutral 书脊）+ 防倒卖卡（warn 书脊）。搜索中或有结果时自动让位。硬编码，模板内有详细的更新指引注释。部分界面文案同步修正（08-10）。 | ✅ |
...
| #   | 项目           | 说明                                                                                                                                                     | 状态 |
|:---|:---|:---|:---|
| 3.1 | 点击引导性不足 | ~~`?` 只悬停不点击。~~ 已修（第 7 步）。 | ✅ |
| 3.2 | 悬浮提示 BUG   | ~~hover 显示 glossary 而非 aria-label。~~ 已修（第 7 步）。 | ✅ |

### 四、TopBar

| #   | 项目           | 说明                                                                                                                                                     | 状态 |
|:---|:---|:---|:---|
| 4.1 | 页签等宽 | 三个页签「搜索/已安装/设置」宽度随字符数各异，指示器滑移时宽度跳变明显。已修：`min-width: 4.5em` + `text-align: center`，覆盖最宽的「已安装」，短页签自动居中（08-10）。 | ✅ |
| 4.2 | 窗控按钮热区太小 | 已修：加 `height: 100%`（08-09）。 | ✅ |

### 五、UiProgress（新增 · 08-09）

| #   | 项目           | 说明                                                                                                                                                     | 状态 |
|:---|:---|:---|:---|
| 5.1 | UiProgress 前端实装 + 库空态修复 | `UiProgress.vue` 零调用点，落点是宪法 5.5「试下载占位行 + 逐条就地替换」。计划与库空态 ContentPane 高度问题（2.1）一起处理。 | 待做 |
| 5.2 | 后端进度推送 | 后端零处 `EventsEmit`，需在 `refactor/v2` 分支加推送，与 5.1 配套。 | 待做 |
