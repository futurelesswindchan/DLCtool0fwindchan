...
4. ✅ `SearchPane` 换原语、`GameCard` 删 grid、`DlcList` 换 `UiCheckbox`
   （08-05）。**唯一遗留：`UiCheckbox` × 200 的性能实测待在实机上跑**
...
| `UiCheckbox` 大列表性能 | 代码已换（08-05），**实测待跑**。每行元素数 6→12，其中 4 个 SVG 节点；200 个 DLC 即整表 2400 元素 / 800 SVG。复现入口：`#/dev/ui` 的「压力档 · UiCheckbox × 200」开关，实机样本用 MHW(582010)。若确认偏慢，退路是给 `.cb__box` 内的两个 svg 换成伪元素或按需渲染（仅选中行画勾） |
